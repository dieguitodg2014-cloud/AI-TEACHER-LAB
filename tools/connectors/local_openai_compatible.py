...
def _build_prompt(generation_request: dict[str, Any], previous_errors: list[str]) -> str:
    ...
    return (
        ...
        "assessment_link field, but you MAY rewrite a field when its wording causes a QC error. "
        "Do not preserve defective wording merely because it appeared in the approved plan. "
        ...
        "For OBJECTIVE_PRODUCTION_MISMATCH, compare the action required by the objective with the "
        "student_production fields. If the objective requires oral communication, explicitly state "
        "an observable oral action such as speaking, talking, discussing, interviewing, presenting, "
        "describing orally, or sharing information. If the objective requires written production, "
        "explicitly state an observable writing action such as writing, completing, filling in, or "
        "composing. Do not use vague wording such as 'produce language' or 'demonstrate the objective' "
        "when a more specific observable action is required. Make the correction in the lesson fields, "
        "not in explanatory commentary.\n\n"
